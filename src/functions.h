...
#define BEACON_PERIOD_MAX_MS 5000
...
uint16_t beaconPeriod = 1000;
uint32_t nextBlinkAt;
uint32_t releaseAt;
uint32_t configLastChangeTime;
...
struct Beacon
{
    ...
    void startBurst()
    {
        ...
        state = BURST;
        t0 = millis();
        blinker.startEx(1, BEACON_ON_MS, 0, 0, 0, false);
        DBG("Sending beacon...\n");
    }
    ...
    void tick()
    {
        ...
        if (state == BURST && dt >= BEACON_ON_MS)
        {
            stopBurst();
        }
        else if (state == GAP && dt >= beaconPeriod)
        {
            startBurst();
        }
    }
...
struct Parrot
{
    ...
    void tick()
    {
        ...
        case P_MEASURE:
            if (!isStreaming)
            {
                uint32_t dur = millis() - t_on;
                DBG("[MEASURE] RX end: %lu ms, dur=%lu ms\n", millis(), dur);
                if (dur >= MIN_DURATION_MS)
                {
                    t_dur = dur;
                    t_on = millis() + COOLDOWN_MS;
                    state = P_COOLDOWN;
...
struct Scanner
{
    ...
    void tick()
    {
        ...
        if (millis() - t_last >= DWELL_MS && !found)
        {
            rateIdx++;
            if (rateIdx >= 3)
            {
...
struct SOSPlayer
{
    ...
    void tick()
    {
        ...
        case 0:
            ...
            tNext = millis() + SOS_TIME_UNIT + 3 * SOS_TIME_UNIT;
            step = 1;
            break;
...
bool isLowBattery()
{
    ...
    if (millis() >= nextSampleAt)
    {
        ...
        nextSampleAt = millis() + BATT_SAMPLE_MS;
    }
    return low;
}
